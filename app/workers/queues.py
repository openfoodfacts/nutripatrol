...
from app.settings import NUM_RQ_WORKERS
...
high_queues = [
    Queue(f"nutripatrol-high-{i + 1}", connection=redis_conn)
    for i in range(NUM_RQ_WORKERS)
]
low_queue = Queue("nutripatrol-low", connection=redis_conn)
